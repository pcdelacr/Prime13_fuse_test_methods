...
| MaxLyaCount          | No            | Integer         | Maximum number of cells to perform LYA testing on.                                    | Default is 99999                                     |

## MaxLyaCount
This parameter limits the maximum number of cells to perform LYA testing on. If cell filtering configuration is provided, the quantity per defect will not override this value. If the amount of cells to test on is exceeded, no further cells will be tested.
...
![alt text](./.attachments/Lya/ExitPorts.png)
...
# Input files
The LYA test method consumes one JSON configuration file.
  ...
  "LyaConfigurations": [
  {
    ...
    "CellFiltering": {
      "DefectTypes": [
        { "Name":  "S_B", "Quantity":  1 },
        { "Name":  "M_B", "Quantity":  3 },
        { "Name":  "SFR2D", "Quantity":  0 }
      ],
      "BlockRanges": "0-64,320-383"
    }
  }]
}
```
...
- _**LyaConfigurations**_: Grouping field used to define LYA configurations per array name.
  ...
  - _**CellFiltering**_: Optional grouping field. Contains configuration for filtering defects for analysis based on a defect's type and block. Only cells that match to these conditions will be tested on.
    - _**DefectTypes**_: _Optional_* field. Contains an array of parameters that define the defect types to perform analysis on, and the maximum number of defects to analyze per defined defect type.
      - _**Name**_: String value. Represents the type of defect that will be allowed through the filter for further testing. Defects that don't match to this name will not be tested on.
      - _**Quantity**_: Integer value. Represents maximum amount of defects to test on for the defined _"Name"_.
    - _**BlockRanges**_: _Optional_* string value. Contains ranges of integers that defines blocks to analyze. A defect's block must be within the defined ranges to be used for analysis.
    Ranges are expected to be defined as two integers seperated by a dash. Additional ranges can be added by delimiting with a comma. Single integers delimited by commas are also valid for this parameter.
    Ex. "0-63,65,100-200" is a valid block range that defines blocks 0 through 63, 65, and blocks 100 through 200 for analysis.

## Notes
More information on parameters is defined in the LYA json file. 

### CellFiltering
`DefectTypes` and `BlockRanges` are both defined as "optional" parameters, but you must define at least one of these parameters for the CellFiltering configuration to be considered valid.

If neither of these parameters are defined for your configuration, the test method will exit port -1 during Verify().
...
| **0**         | ***Fail***    | Not returned by this test method.                          |
| **1**         | ***Pass***    | All cells returned valid Dc measurements.                  |
| **2**         | ***Pass***    | Number of cells tested exceeded the MaxLyaCount parameter. |
| **3**         | ***Pass***    | At least one cell returned invalid Dc measurements.        |
...
| Jul 16<sup>th</sup>, 2024 | 13.1.0      | Caio Fernandes | Addition of CellFiltering configuration. |