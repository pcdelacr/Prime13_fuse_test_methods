...
namespace PUP
{
    ...
    /// </summary>
    [PrimeTestMethod]
    public class PupCallBacks : PrimeCallbacksRegistrarTestMethod, ICallbacksRegistrarExtension
    {
        /// <summary>
        /// The registration of the callbacks to be used by EVG for PUP purpose.
        /// </summary>
        void ICallbacksRegistrarExtension.RegisterCallbacks()
        {
            this.RegisterCallback(PupCallBacks.IsPlistEligibleForTTR);
        }

        private static string IsPlistEligibleForTTR(string args)
        {
        ...
        private static bool ResolveOptimization(ExcludeInformation information, ref string excludeValue)
        {
            ...
            {
                optimizationResult = ResolveCriteriaBn(values, ref excludeValue);
            }

            // flow number check
            if (name.EndsWith("EXFN"))
            {
                optimizationResult = ResolveCriteriaFlowNum(values, ref excludeValue);
            }
            ...
            if (name.EndsWith("EXVM"))
            {
                ...
                optimizationResult = ResolveCriteriaEntryVmin(values[0], function, comparison, ref excludeValue);
            }

            if (string.Equals(name, "SharedStorageKey"))
            {
                optimizationResult = ResolveCriteriaSharedStorage(values, ref excludeValue);
            }

            if (string.Equals(name, "UserVarName"))
            {
                optimizationResult = ResolveCriteriaUserVar(values, ref excludeValue);
            }
        ...
        private static bool ResolveCriteriaSharedStorage(IList<string> values, ref string excludeValue)
        {
            ...
            foreach (var value in values)
            {
                ...
                if (sharedStorageInformation.Count == 2)
                {
                    ...
                    try
                    {
                        storedValue = Prime.Services.SharedStorageService.GetRowFromTable<string>(storageKey, context);
                    }
                    catch (Exception e)
                    {
                        Console.WriteLine(e);
        ...
        private static bool ResolveCriteriaBn(IList<string> values, ref string excludeValue)
        {
            ...
            foreach (var value in values)
            {
                ...
                try
                {
                    Prime.Services.SharedStorageService.GetRowFromTable<int>(gsdsName, Context.DUT);
                    noMatchingExcludeCriteriaFound = false;
                    excludeValue = value;
                    break;
                }
        ...
        private static bool CompareDouble(double referenceValue, double valueToCompare, string comparisonOperation)
        {
            ...
            else if (string.Equals(comparisonOperation, "lt"))
            {
                return referenceValue < valueToCompare;
            }
            else if (string.Equals(comparisonOperation, "ge"))
            {
                return referenceValue >= valueToCompare;
            }
            else if (string.Equals(comparisonOperation, "le"))
            {
                return referenceValue <= valueToCompare;
            }
            else if (string.Equals(comparisonOperation, "eq"))
            {
        ...
        private static bool ResolveCriteriaEntryVmin(string value, string function, string comparisonOperation, ref string excludeValue)
        {
            ...
            try
            {
                var lastStartVoltages = Prime.Services.SharedStorageService.GetRowFromTable<string>("LastFastSearchInstanceStartVoltage", Context.DUT).
                    Split(',').ToList();
        ...
        private static bool ResolveCriteriaFlowNum(IList<string> values, ref string excludeValue)
        {
            ...
            try
            {
                flow = Prime.Services.SharedStorageService.GetRowFromTable<int>("LastFastSearchInstanceStartVoltage", Context.DUT);
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
        ...
        {
            /// <summary>
            /// Initializes a new instance of ExcludeInformation class.
            /// </summary>
