...
![image.png](./.attachments/FuseReadMaskUltDecode/image-926e05fb-0c2b-4c03-a832-13c5d8c5bced1.png)

When captured bit count is 56, the code is generated following this conversion:

![image.png](./.attachments/FuseReadMaskUltDecode/image-0e9d3844-eeee-4d11-b038-1d47b5b66d8f1.png)

When captured bit count is 64, the code is generated following this conversion:

![Universal intel ULT decode.png](./.attachments/FuseReadMaskUltDecode/Universal%20intel%20ULT%20decode-e936fbde-838c-4a0c-9bdd-3e697d14afce1.png)
...
| FuseGroupToDatalog             | No            | CommaSeparatedString | Valid register FuseGroupName. | Default is empty. Use to datalog the fuse string of the FuseGroupName. See [Special Cases](#special-cases) for details. |
...
| PrintUltDataPerDieIdToItuff    | No            | String (choice) | DISABLED, ENABLED (Default)                              | By default Ult info per die ID will print to ituff. DISABLED for SCP and to print `2_psnbinary` to ituff. |
...
| DummyStorageKey                | No            | String          | Shared Storage key name to store dummy registers.  | Default is empty.|
| UploadUltToDffService          | No            | String (choice) | DISABLED, ENABLED (Default)                              | DISABLED indicate no upload decoded ULT to Dffservice, ENABLED will upload the Decoded ULT to DffService and there is a check with ULT vs DieID. |
...
2. VID Verification
         ...
         This is an optional sanity check options. it's to check the Visual ID from UBE file and SCVARS.SC_VISUALID are match, by using the decoded ULT and Die ID declared, and print the result to ituff.
         This check is depends on an uservar named DFFVars.VerifyVIDToEFuse, it only works when the uservar is exist and with "ENABLED" value.

        This is when the Visual IDs are match.
        ```
        2_tname_<TestInstanceName>_pass_ExpectedValueFromUBE
        2_tssid_<DieID>
        2_strgalt_nsv_<Ult>
        2_tname_<TestInstanceName>_pass_ValueFromFuseread
        2_tssid_<DieID>
        2_strgalt_nsv_<Ult>
        ```

        This is when the Visual ID is not match.
        ```
        2_tname_<TestInstanceName>_fail_ExpectedValueFromUBE
        2_tssid_<DieID>
        2_strgalt_nsv_<correct Ult>
        2_tname_<TestInstanceName>_fail_ValueFromFuseread
        2_tssid_<DieID>
        2_strgalt_nsv_<decoded Ult>
        ```

## Parameters relation

![image.png](./.attachments/FuseReadMaskUltDecode/parameterRelationTable.png)
...
```c#
        ...
        void IFuseReadMaskUltDecodeExtensions.CustomExecute(FuseReadMaskUltDecodeTestInstanceResults fuseReadTestInstanceResults)
        {
            ...
            {
                ((IFuseReadMaskUltDecodeExtensions)this).ExecuteFailingMask(fuseReadTestInstanceResults);
            ...
            {
                ((IFuseReadMaskUltDecodeExtensions)this).ExecutePassingMask(fuseReadTestInstanceResults);
            }
        }
```
...
```c#
        ...
        void IFuseReadMaskUltDecodeExtensions.CustomDatalogFuseStringByFuseGroupName(IFuseReadHandle fuseReadHandle)
        {
            ///////////////////////////////////
            ...
            if (!string.IsNullOrEmpty(this.FuseGroupToDatalog))
            {
                foreach (var fuseGroupName in this.FuseGroupToDatalog.ToList())
                {
                    ...
                    if (fuseReadHandle.RegisterDieIdName == "NA")
                    {
                    ...
                    {
                        FuseDataManager.SharedStorage.StoreFuseGroupToDatalogToSharedStorage(fuseGroupName, fuseReadHandle.RegisterDieIdName.Replace(".", string.Empty), fuseStringOfFuseGroup, this.SessionContext);
                    }
...
```c#
        ...
        {
            if (this.postUltMaskNames.Count != 0)
            {
                ...
                foreach (var maskName in this.postUltMaskNames)
                {
                    ...
                    {
                        Base.ServiceStore<IConsoleService>.Service.PrintError($"Mask execution failed for MaskName=[{maskName}].", this.SessionContext);
...
MiniFlow only supported by FuseReadMaskUltDecode TestMethod.
[Download](./.attachments/FuseReadMaskUltDecode/MiniFlowUserReference.cs)
...
|ULTDecode() | Execute UltDecode that defined in configuration file. | Bool. True for successful execute ult decode, otherwise False. |
|ReadMask(string maskName) | Same as FuseRead ExecuteMask. Compare fusestring with provided Mask value. The name of mask must be valid and exist in configuration file. | Bool. True for fusestring matched with mask value, otherwise False. User is required to setup mask information in configuration file. |

[Download File-based miniflow Guide](./.attachments/FuseRead/PrimeMiniFlow_UserGuides.pptx)
...
![UserVar2DieIDWithNoPackEfuse.png](./.attachments/FuseReadMaskUltDecode/UserVar2DieIDWithNoPackEfuse.png)
...
![UserVar2DieIDWithPackEfuse.png](./.attachments/FuseReadMaskUltDecode/UserVar2DieIDWithPackEfuse.png)
...
Execute UltDecode only, with no mask, by leaving MaskName and PostUltExecuteMaskName parameter empty and disable ult upload to DffService.
<pre><code>Test PrimeFuseReadMaskUltDecodeTestMethod ExecutePattern_x2_UltDecode_NoMask_Pass_P1
{
	ConfigName = "Config24";
	<span style="background-color:yellow">MaskName = "";</span>
	...
	<span style="background-color:yellow">PostUltExecuteMaskName = "";</span>
	PackageEfuse = "U4";
	<span style="background-color:yellow">UploadUltToDffService = "DISABLED";</span>
}
</code></pre>
...
![VerifyFlowChart.png](./.attachments/FuseReadMaskUltDecode/VerifyFlowChart.png)

Main Execute Flow

![MainExecuteFlowChart.png](./.attachments/FuseReadMaskUltDecode/MainExecuteFlowChart.png)
...
![ExecuteFailingMask.png](./.attachments/FuseReadMaskUltDecode/ExecuteFailingMask.png)   ![ExecutePassingMask.png](./.attachments/FuseReadMaskUltDecode/ExecutePassingMask.png)

![UltCustomExecuteFlowChart.png](./.attachments/FuseReadMaskUltDecode/UltCustomExecuteFlowChart.png)

![PostUltCustomExecuteFlowChart.png](./.attachments/FuseReadMaskUltDecode/PostUltCustomExecuteFlowChart.png)

![CustomDatalogFuseStringByFuseGroupNameFlowChart.png](./.attachments/FuseReadMaskUltDecode/CustomDatalogFuseStringByFuseGroupNameFlowChart.png)
...
| ------------- | -------------------- | ------------ |
| 13.04.00      | #62510               | FuseGroupToDatalog should follow per register per DieID pairing. |
| 13.03.00      | #58418               | Implement File-Based MiniFlow. |
| 13.03.00      | #61911               | Add lock to UltHandler.SaveUltStringToSharedStorage to solve racing condition when parallel execute multiple fuse read test instance. |
...
| 13.03.00      | #59057               | PrimeFuseReadMaskUltDecodeTestMethod - psnbinary printing in ITUFF |
| 13.01.00      | #48664               | FuseMiniFlow by Extension. |
| 13.01.00      | #50665               | Adding Feature VID verifaction.|
