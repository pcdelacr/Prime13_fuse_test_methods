...
```
1 (cycle from V) + 4 (cycles from first S) + 4 (cycles from second S) + 3 (link count of S3 in the third S vector) = 12
```
...
```
3. The `DSS_LIMIT_CATASTROPHIC` string will be printed at each of the patterns when at least one limit (TotalFailCaptureCount or PerPatternFailCaptureCount) has been reached. The pattern name will be printed to the console log in the case the per pattern limit is reached.
...
| Oct 13<sup>th</sup>, 2024  | 13.1 | Lee, Yeong Jui | [Need to fix DSS_LIMIT_CATASTROPHIC to printed at the point in which limit has reached either per pattern or total](https://dev.azure.com/mit-us/PRIME/_workitems/edit/51655/) |
...
| Jul 18<sup>th</sup>, 2025 | 13.3.3 | Tiow, Hian Seng | [[PrimeScanSpofiTestMethod]EngineeringSpofiOutputFolderPath filename with invalid syntax](https://dev.azure.com/mit-us/PRIME/_workitems/edit/59878/) |
