...
![Dynamic flow execute](../.attachments/RepairToFuse/ExecuteFlow.png)
...
| FuseData=[\<FuseData value>\] cannot be inserted as FuseMap=[\<FuseMap name>\] is fulled with previous fuse data <br>Note: Exits port 2 | FuseMap pointed at by the process resource is already full with previous data <br>The number of fuses is less than the number of repair resources to be written| Review ProcessResource and make sure that is pointing to the correct FuseMap <br/> Review the FuseData format to make sure it has the correct size | Review the MapValues defined in the configuration file and make sure there are enough fuses to store all the repair resources |
| Cannot insert fuseData=[\<FuseData value>\] into fuse value because there is no space left to insert the data <br>Note: Exits port 2 | FuseMap is not yet full but FuseData size is longer than the amount of available spaces in the final binary string <br>The number of fuses is less than the number of repair resources to be written| Review ProcessResource and make sure that is pointing to the correct FuseMap <br/> Review the FuseData format to make sure it has the correct size | Review the MapValues defined in the configuration file and make sure there are enough fuses to store all the repair resources |
| Cannot insert fuseData=[\<FuseData value>\] at fuseIndex=[\<FuseIndex value>\]. The calculated starting index for insertion=[\<Fuse start index>\] exceeds or equals the total fuse space length=[\<Total fuse space>\]. | The specified fuse index or indexes surpass the combined limits of the total number of available fuses and the designated size of the FuseData. | Review the FuseIndex values defined in the configuration file to ensure they are appropriate for the number of fuses outlined in the FuseMap and the size constraints of the FuseData |
...
![Static flow execute](../.attachments/RepairToFuse/FileConnection.png)
...
| 2 | Fail | Insufficient space for fuse data |
