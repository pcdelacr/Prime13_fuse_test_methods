...
![image.png](../.attachments/ScanHRYSsn/SSNHryVerifyFlowchart.png)

Execute Flowchart

![image.png](../.attachments/ScanHRYSsn/SSNHryExecuteFlowchart.png)
...
| HPTPMode               | No. Default is Normal. | String (Choice) | Normal mode will use regular SSN to decode. Other two options are: HPTP and DUAL_HPTP. | Default = Normal. |
...
- 0 failing partition
- 1 passing partition
- 8 untested partition 
  - when TotalFailCaptureCount limit is reached in current plist execution, all partitions that are not failing will be marked as 8
- 9 unassigned partition 
  - no section in the input file point to this partition index. In other words, no "HRYIndex" points that partition
...
Below is another example that uses te optional field 'ParentPlistRegex'. This is mainly used in products in which they cover all cores in single plist, and differntiating them through sub-plists.
```json
{
    "HRYLength": 6,
    "Patterns": [
        {
            "PacketSize": 54,
            "OutputPacketOffset": 10,
			"ParentPlistRegex": ".*core1.*"
            "PatternRegex": ".*reset*",
            "Partitions": [
                {
                    "BitPositions": "0-53",
                    "HRYIndex": 0,
                    "HRYPrint": "reset-core1"
                }
            ]
        },
		{
            ...
            "Partitions": [
                {
                    "BitPositions": "0-12,13,14-16",
                    "HRYIndex": 2,
                    "HRYPrint": "pariirp-core1"
                },
                {
                    "BitPositions": "17-53",
                    "HRYIndex": 3,
                    "HRYPrint": "pariiommu-core1"
                }
            ]
        },
		{
            ...
            "Partitions": [
                {
                    "BitPositions": "0-12,13,14-16",
                    "HRYIndex": 4,
                    "HRYPrint": "pariirp-core2"
                },
                {
                    "BitPositions": "17-53",
                    "HRYIndex": 5,
                    "HRYPrint": "pariiommu-core2"
                }
            ]
        }
    ]
...
- _**Patterns**_: Grouping field. Used to provide a list of pattern Scan description.
  ...
  - _**SSNBusWidth**_: Optional field only taking effect when HPTP mode is chosen. This will overwrite SSNBusWidth and PinNumbers in pinMapping file and taking into HPTP equation as the bus size. Normal SSN mode does not affect by this field.
  - _**ParentPlistRegex**_: Optional field to provide a regex to match parent plist of the failing pattern. When not provided, only PatternRegex will be used for matching this section.
...
The general formula for HPTP decode is stated as below:
$$
SSN\_Position = Starter\_Bit + HPTP\_Row + (Pin\_position * 2)
$$
...
If HPTP Vector = 1, HPTP Row = +16

If HPTP Vector = 2, HPTP Row = +1

If HPTP Vector = 3, HPTP Row = +17


To determine pin position multiplier:
$$
Pin\_Position\_with\_multiplier = Pin\_position * 2
$$
...
3. To determine pin position multiplier:
$$
Pin\_Position\_with\_multiplier = 3 * 2
$$
$$
Pin\_Position\_with\_multiplier = 6
$$
...
4. To determine final ssn position:
If Pin_Position > 8, do this:
$$
Final\_SSN\_Position = SSN\_Position + 16
$$

In our example, Pin_Position is 3 so we don't add 16 to calculate final SSN Position.

### Exception for both HPTP modes:
...
| May 10<sup>th</sup>, 2024 | 13.0.0 | Tiow, Hian Seng | [[SCAN] ScanSSNHRY new feature to support HPTP](https://dev.azure.com/mit-us/PRIME/_workitems/edit/46839/) |
| May 22<sup>th</sup>, 2024 | 13.1.0 | Wajde, Dakwar | [[ScanHRYSSN] Code does not support pattern regex + plist regex which means we cannot use for multi-instances which re-use patterns](https://dev.azure.com/mit-us/PRIME/_workitems/edit/49743/) |
| July 18<sup>th</sup>, 2024| 13.1.0 | Tiow, Hian Seng | [[ScanHRYSSN] Unexpected port 0 error unable to match object with failing position to pattern regex](https://dev.azure.com/mit-us/PRIME/_workitems/edit/51838/) |
| Mar 13<sup>th</sup>, 2025 | 13.2.0 | Tiow, Hian Seng | Correction on both ScanLYA & ScanHRYSSN HPTP calculation. [[ScanLYA] Enable single-plist-only execution with capability to control SPOFI logging](https://dev.azure.com/mit-us/PRIME/_workitems/edit/54470/)|
