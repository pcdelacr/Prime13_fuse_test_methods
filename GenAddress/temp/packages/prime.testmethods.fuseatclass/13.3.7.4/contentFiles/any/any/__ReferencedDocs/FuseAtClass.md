...
This **REP** is intended to describe the PrimeFuseAtClassTestMethod.
...
-->FactModeOff{Does fact mode equals FactMode_OFF?}--> |No| ParseContext[Validate that the shared storage<br>keys contain the Context] 
 --> ValidateOptypeFC[Validate the OpType is defined and the OpType in DFF matches the one in the user var]
...
ValidateMappingGsdsFC-->ReadLrf[Read and parse LRF <br>and FRF files]
     -->  ValidateMrvUserVarParam[Validate the Mrv, revision, mode, <br>frv parameters from the input files<br> and check if the version matches<br> depending on the VersionMatch parameter]
...
::: mermaid
graph TB
style Execute fill:#00AEEF,stroke:#0000,stroke-width:2px,color:#fff
style FactModeOff fill:#0071C5,stroke:#0000,stroke-width:2px,color:#fff
...
-->FactModeOff(FactMode == FactMode_OFF)--> |Yes| PASS[Return pass]
FactModeOff -->|No|CheckMrv[Check the Mrv, lot id, device, package, assembly revision and step]
-->LogBinMapping[Save SspecBinsMappingObject to the shared storage]
-->PopulateTsAttrs
--> PASS --> End(End)
:::
...
The table below lists and describes the test instance parameters supported by the PrimeFuseAtClassTestMethod:
...
![image.png](./.attachments/FuseAtClass/AssemblyNameDescription.png)
...
- Validations:  
The FRF data validations are performed exclusively on BOMs that correspond to the LRF and fall under a "Finished Good" name matching any of the LRF sspec values.
...
```
## User Var population
The following TSATTRSVars User Vars are populated during the PrimeFuseAtClassTestMethod execution.  
...
| **FaCTMode**         | Value obtained using the "FactMode" test instance parameter. |
...
| Aug 8<sup>th</sup>, 2025  | 13.3.0        | Tiow, Hian Seng      | #62102 Fix UserVarHandle when processing filePath. |
