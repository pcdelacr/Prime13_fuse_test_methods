...
namespace IntegrationTestUserCode.FuseBurn
{
    ...
    /// <summary>
    /// Example of plugin. This plugin will mimic default fuse burn mask execution, ULTEncode and comparing storage value.
    /// </summary>
    [PrimeTestMethod]
    public class MiniFlowUserReferenceBurnMask : PrimeFuseBurnMaskTestMethod, IFuseBurnMaskExtensions
    {
        private ISessionContextProviderContainer sessionContext;

        /// <summary>
        /// Initializes a new instance of the <see cref="MiniFlowUserReferenceBurnMask"/> class.
        /// </summary>
        ...
        public MiniFlowUserReferenceBurnMask(IPatConfigService patConfigService, IConsoleService consoleService, IPinService pinService, Prime.FuseDBService.IFuseDBService fuseDBService, IFunctionalService functionalService)
            : base(patConfigService, consoleService, pinService, fuseDBService, functionalService)
        {
        }

        /// <inheritdoc />
        int IFuseBurnMaskExtensions.ExecuteMiniFlow(IFuseBurnMaskMiniFlow flow)
        {
            this.sessionContext = flow.SessionContext;

            var exitPort = ExecuteMask();

            flow.Datalog($"ExecuteBurnMask_{flow.RegisterName}_Port_{exitPort}");
            ...
            {
                // ExpectOnTrue.
                return flow.MaskBurn("burn_0_full_string") ? ExecuteBurnMaskB() : flow.ExitPort(2);
            }

            // Flow Item.
            int ExecuteBurnMaskB()
            {
                // Expect onTrue.
                return flow.MaskBurn("burn_1_full_string") ? ExecuteULTEncode() : flow.ExitPort(2);
            }
