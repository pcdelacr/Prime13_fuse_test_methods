...
The Electrical Z Alignment test method is intended to be used by the Station Controller to automatically adjust the initial position of the tester's wafer chuck until all pins of the DUT make contact with the wafer bumps.  
The user var `TP_ZHEIGHT_INST` should contain the name of an instance of this test method. TOS will then trigger the execution of this instance.  
This Test Method (TM) conducts DC opens and shorts tests, recording the outcome in the `SC_EZA_TEST_RESULT` user var. It also logs the identifiers of pins that fail the opens test in the `SC_EZA_OPEN_PINS` user var. TOS then uses these user vars to decide whether the test instance should be triggered again or not.  
...
### Execute
![image.png](./.attachments/ElectricalZAlignment/EZA_Execute.png)

## Test Instance Parameters

The table below lists and describes the test instance parameters supported by the Electrical Z Alignment test method

| **Parameter Name** | **Required?** | **Type**        | **Values**                                                                           | **Comments**                 |
| ------------------ | ------------- | --------------- | ------------------------------------------------------------------------------------ | ---------------------------- |
| LevelsTc           | Yes           | LevelsCondition | Levels TC required to setup and perform the Electrical Z Alignment measurement.                           |                              | |
| SetupFilePath       | Yes            | String | Path to the json setup file. | 
| ItuffDataloggingEnabled       | No            | String | TRUE or FALSE. | ITUFF datalogging can be disabled to prevent issues if ITUFF has not been properly set up.
...
## UserVars
This TM uses several UserVars:

| UserVar                          | Description                                                                                                     |
|----------------------------------|-----------------------------------------------------------------------------------------------------------------|
| SCVars.SC_ELECZHTFSTPN           | Z-position at which first probe-to-wafer contact was established.                                               |
| SCVars.SC_ELECZHT                | Final Z-position determined by the Electrical Z Height algorithm.                                               |
| SCVars.TP_ZHEIGHT_FAILPINS_LIMIT | Maximum number of pins failing the Electrical Z Alignment test that should be on the SC_EZA_OPEN_PINS user var. |
| SCVars.SC_EZA_OPEN_PINS          | Comma separated list of pins failing the Electrical Z Alignment test.                                           |
| SCVars.SC_EZA_TEST_RESULT        | Result of executing the Electrical Z Alignment test.                                                            |
| SCVars.SC_LOT_WAFER              | Lot of the wafer.                                                                                               |

Any of those uservars missing can cause issues or default values to be used when running the TM. If you find any issues refer to the previous uservars to be available.

## Datalog output

Electrical Z Alignment results results are logged to ITUFF using the “composite” format as shown below. The data is logged as follows: `composite_{Electrical Z Height Value}_{Electrical Z Alignment Pin Value}`.
```python
...
When the shorts and/or opens fail, the failing channels are printed to the ITUFF as shown below.
...
2_faildata_8015
```
Additionally, the identifiers of the failing pins are also logged in ITUFF, as follows:
```python
...
```
Note: The fields OpensMask and ShortsMask are optional.

## Exit Ports

The Electrical Z Alignment test method supports the following exit ports:
...
| **1**         | ***Pass***    | All opens and shorts tests pass condition.          |
...
| Aug  22<sup>nd</sup>, 2024 | Prime 14.00.00 | Raquel Pinto Rosales | Documentation enhancement |