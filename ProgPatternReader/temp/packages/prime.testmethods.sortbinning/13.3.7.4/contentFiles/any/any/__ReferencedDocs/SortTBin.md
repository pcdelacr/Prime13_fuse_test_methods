...
![SortTBin Flow](./.attachments/SortTBin/TBinFlow.drawio.png)

</center>

#### Note: Before applying a bin the code will check if a reserved bin is currently set by TOS, such as an alarm bin. If that is the case no bin will be set, but the rest of the logging logic will still execute with the calculated bin.
...
If the Current Bin is 98 or 99 and that is not one of the BinInputs provided then the Current Bin will be assigned as the final Bin.

If the Current Bin is included in the provided BinInputs then the standard TBIN logic will determine which flow failed.
...
![Bad Die Binning Flow](./.attachments/SortTBin/TBinBadDieBinningFlow.drawio.png)
...
![Good Die Binning Flow](./.attachments/SortTBin/TBinGoodDieBinningFlow.drawio.png)
...
FTBin = (ITBin * 100) + (IncomingIbins[1] * 10) + (IncomingIbins[0])
...
FTBin = (6 * 100) + (<span style="color:#996600">0</span> * 10) + (<span style="color:#FF3300">2</span>) = 602

DTBin = (602 * 10000) + (<span style="color:#00AAFF">3</span> * 1000) + (<span style="color:#FF3300">0</span> * 100) + (<span style="color:#FF9900">6</span> * 10) + (<span style="color:#9966FF">5</span> ) = 6023065
