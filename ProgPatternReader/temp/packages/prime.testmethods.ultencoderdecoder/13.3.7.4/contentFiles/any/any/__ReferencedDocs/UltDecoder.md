...
| DieIdNames                  | Yes           | String                                      | Comma separated list of the die id associated with each of the pins. You must have one for each pin. Ex. U1,U2.U1,U3. Also accept uservar in replace of die id. Ex. UserVar.DieId1,U2.U1,UserVar.DieId2. However one uservar should only represent one die id. | When DieIdName <font color="red">**match** </font> PackageEfuse, ULT info will print to ituff as trlot, trwafer, trxloc, tryloc.<br>When DieIdName <font color="red">**not match**</font> PackageEfuse or PackageEfuse is empty, this ituff will not printed.<br>See [ULT Ituff scenario](#ult-ituff-scenario)|
| PackageEfuse                | No            | String                                      | DieId Name to print ituff as PackageEfuse.                                                                                                                                                                                                                     | PackageEfuse is used with DieIdNames. See [ULT Ituff scenario](#ult-ituff-scenario)|
| PrintUltDataPerDieIdToItuff | No            | String (Choice) ENABLED (Default), DISABLED | Set as <font color="red">**ENABLED**</font> to print ULT info to ituff as sstrlot, sstrwafer, sstrxloc, sstryloc.<br>Set as <font color="red">**DISABLED**</font> to not print this ituff.                                                                     | See [ULT Ituff scenario](#ult-ituff-scenario)|
...
| CtvOrder                    | Yes           | LSB/MSB                                     | This denotes whether the first bit is the least significant [LSB]  or the most significant [MSB]. Default is LSB                                                                                                                                               |              |
| UltDecoderPluginName        | No            | String                                      | This parameter specifies the plugin name to use to decode the data. Default=[`UltDecoderPlugin`]. Prime available Plugins: `UltDecoderPlugin` or `TsmcUltDecoderPlugin`                                                                                        |              |
| UltPluginUserInput          | No            | String                                      | This parameter specifies optional input data required for the Plugin denoted by `UltDecoderPluginName`                                                                                                                                                         |              |
...
![image.png](./.attachments/UltDecoder/image-926e05fb-0c2b-4c03-a832-13c5d8c5bced.png)

When capture bits count is 56, the code is done following this conversion:

![image.png](./.attachments/UltDecoder/image-0e9d3844-eeee-4d11-b038-1d47b5b66d8f.png)

When capture bits count is 64, the code is done following this conversion:

![Universal intel ULT decode.png](./.attachments/UltDecoder/Universal%20intel%20ULT%20decode-e936fbde-838c-4a0c-9bdd-3e697d14afce.png)
Intel Universal decoding.

## TSMC's decoding
It's implemented and exposed via the `TsmcDecoderPlugin`.

![TSMC.png](./.attachments/UltDecoder/TSMC-4ad66bd1-697d-4153-8d43-6529bc3bb74e.png)

## Console output

When LogLevel is TEST\_METHOD - test method shows the actual CTV data captured

![image.png](./.attachments/UltDecoder/image-735976f4-e279-4785-8ebb-eb763b49c433.png)
...
### ULT Ituff scenario
Condition/Switches that control ituff ULT ituff.
![PackageEfuse.png](./.attachments/UltDecoder/PackageEfuse-condition.png)

![PrintUltDataPerDieIdToItuff.png](./.attachments/UltDecoder/PrintUltDataPerDieIdToItuff-switch.png)

Scenario
![ULTItuffScenario.png](./.attachments/UltDecoder/ULT-ituff-scenario.png)
...
UserVar DFFVars.VerifyVIDToEFuse is introduced to check visual ID from UBE file. The UserVar is allowed be not found, and default as DISABLED of the checking. In the code, the function will use the decoded Ult to grab the UNIT value from UBE file, and check with the SCVars.SC_VISUAL_ID. It will flag out when the UNIT value not found in UBE file, or the Visual ID is mismatch.
...
| Sep. 26, 2023 | 13.1.0      | Lee, Yeong Jui    | Adding Feature VID verifaction                                     |
| May  21, 2025 | 13.3.0      | Teoh, Khai Jie    | Improve documentation for test instance parameter DieIdNames, PackageEfuse, PrintUltDataPerDieIdToItuff. Add ULT Ituff scenario. |
