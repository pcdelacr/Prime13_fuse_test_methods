...
# Service details
The tracking information is saved per dieId within a tracking data structure like the following example:
...
The data flow items saved for each dieId keep the order in which they are added in this tracking structure because later the bin is decided based on the last flow item which saved port has a bin assigned for the corresponding dieId.
...
```csharp
    {U1, 90424253}, {U1.U1, 100}, {U2, 90444446}
```
...
- When using SavePortInformationFromCurrentDieId, if previously current Die Id was not set correctly (die id is not part of Active Die Id, or invalid Die Id), it will fail this execution. This can be reset by setting a valid Die Id again using SetCurrentDieId.
