...
Ramp methods takes same inputs: list of TDAU pins (one per card), temperature set point(s), a temperature guard-band and offset in order to check when the set-point has been reached, a timeout in mili-seconds is also provided in case temperature can’t be reached.
