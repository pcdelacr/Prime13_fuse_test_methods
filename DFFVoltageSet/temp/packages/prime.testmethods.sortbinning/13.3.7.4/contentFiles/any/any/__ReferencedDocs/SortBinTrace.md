...
Additionally, the BinTrace that is turned off will be collected and logged completely to the ituff as a DEFLATE32 message. This message will contain the BinTrace data for each instance. 

Example:
```python
2_tname_SortBinning::BinTrace_BinTraceOff_P1_BINTRACETEST
2_strgval_DEFLATE32_BPHC6KTRZLGMXS6MJO33F4WJJ5GESLMKB5F4ZSKMOHHM6S6LJQXS2SRMZHGM646LZREY2DZQ2QY3GNFTGQYDAMGUGGWASRWWDCOAUYXD2RUKEY3JMBUWC2IB2RUAEAA=
```

The tname format is as follows:
```2_tname_{instanceName}_{BinTraceNameThatWasClosed}```

Decompressing the previous message (using Prime Error Decoder) generates:
```python
--TestName(tname)=[SortBinning::BinTrace_BinTraceOff_P1_BINTRACETEST]:
SortBinning::Loader_ValidConfigurationFile_P1,69690001,1|SortBinning::SetBin_ValidConfigurationFile_P4,90989801,4
```

Output format for the data is:
```python
{instanceName},{binNumber},{exitPort}|....
```
If given the chance no BinTrace data is found for the trace being closed there will be no ituff print.
